...
describe('Test House Reducer', () => {
  ...
  it('Returns Map from no action', () => {
    ...
    expectIs(reducer(), new Map());
  });

  let map = new Map();
  it('Returns Map from unknown action', () => {
    let action = { type: 'INIT_REDUX' };
    expectIs(reducer(map, action), new Map());
  });

  // let map = new Immutable.Map()
  it('GET_ALL_HOUSES', () => {
    let action = { type: 'GET_ALL_HOUSES' };
    expectIs(reducer(map, action), new Map({ getting_all: true }));
  });

  it('GET_ALL_HOUSES_ERROR after GET_ALL_HOUSES', () => {
    let action = { type: 'GET_ALL_HOUSES_ERROR', error: 'Fetch Error' };
    map = new Map({ getting_all: true });
    expectIs(reducer(map, action), new Map({ error_all: 'Fetch Error' }));
  });
  ...
  it('GET_ALL_HOUSES after GET_ALL_HOUSES', () => {
    let action = {
      type: 'GOT_ALL_HOUSES',
      allHouses: new Map({
        '1': new Map({ name: 'House Stark of Winterfell' }),
      }),
      allHousesIds: new List(['1']),
      regions: new Set(['The North']),
    };
    expectIs(
      reducer(map, action),
      new Map({
